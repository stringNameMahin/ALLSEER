...
// event has either a path, a destination, or a privilege change, never two of
// them, so sensitive_path cannot contribute alongside sensitive_host,
// uncorrelated_destination, or privilege_change. Summing the declared weights is
...
func TestSensitivityDoesNotEscapeTheScale(t *testing.T) {
	...
	if ceiling != 220 {
		t.Errorf("scorer ceiling = %v, want 220", ceiling)
	}
